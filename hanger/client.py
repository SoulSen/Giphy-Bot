...
from hangups.hangouts_pb2 import StateUpdate, GetEntityByIdRequest, EntityLookupSpec, GetConversationRequest, \
    ...
    FIELD_MASK_DEVICE, FIELD_MASK_MOOD, FIELD_MASK_AVAILABLE, FIELD_MASK_REACHABLE
...
class Client:
    def __init__(self, cookies):
        ...
        self._event_handler.register_event('on_group_link_sharing_modification', self.on_group_link_sharing_modification)
        self._event_handler.register_event('on_history_modification', self.on_history_modification)
            ...
            if event_.HasField('chat_message'):
                ...
                if event_.membership_change.type == MEMBERSHIP_CHANGE_TYPE_JOIN:
                    ...
                    await self._event_handler.invoke_event('on_participant_join',
                                                           ParticipantJoinEvent(event_, user, conversation, participants))
                else:
                    ...
                    if user in participants:
                        await self._event_handler.invoke_event('on_participant_leave',
                                                               ParticipantLeaveEvent(event_, user, conversation, participants))
                    else:
                        await self._event_handler.invoke_event('on_participant_kick',
                                                               ParticipantKickEvent(event_, user, conversation, participants))
                ...
                await self._event_handler.invoke_event('on_conversation_rename', ConversationRenameEvent(event_.conversation_rename, user, conversation))

            elif event_.HasField('hangout_event'):
                participants = [conversation.get_participant(user.gaia_id) for user in event_.membership_change.participant_ids]
                await self._event_handler.invoke_event('on_hangout', HangoutEvent(event_, user, conversation, participants))

            elif event_.HasField('group_link_sharing_modification'):
                await self._event_handler.invoke_event('on_group_link_sharing_modification', GroupLinkSharingEvent(event_, user, conversation))

            elif event_.HasField('otr_modification'):
                await self._event_handler.invoke_event('on_history_modification', OTRModificationEvent(event_, user, conversation))
